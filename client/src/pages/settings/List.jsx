...
const List = () => {
  ...
  const [updatedWord, setUpdatedWord] = useState(false)
  ...
  useEffect(() => {
    if (updatedWord && isSuccess) {
      toast.success('Updated word has been saved to the bank.')
    }
  }, [updatedWord])
          ...
          {wordBank.map((item, idx) => {
            return (
              <li
                key={item._id}
                id={item._id}
                className={isActive === idx ? "word editing" : "word edit"}
              >
                <div
                  className="itemWord"
                ...
                <div className="editSaveDelete">
                  <div className="btn edit" onClick={e => changeToEdit(e, idx)}>
